<script lang="ts">
    ...
    async function handleRegister(e: SubmitEvent) {
        ...
        try {
            ...
            if (error) {
                alertRef.show("Registration failed: " + error.message, "error");
            } else {
                alertRef.show(
    ...
    async function loginWithGoogle() {
        ...
        try {
            ...
            alertRef.show(
                "Registration with Google failed: " + err.message,
                "error",
            );
        } finally {
            loading = false;
    ...
    <div class="card-body">
        <div class="mb-8 text-center">
            ...
            <p class="text-base-content/60 font-medium">
                Create your studio account today.
            </p>
        </div>
        ...
        <form class="space-y-5" onsubmit={handleRegister}>
            ...
            <p class="text-sm text-center opacity-60 font-medium">
                Already have an account? <a
                    href="/login"
                    class="text-primary font-bold hover:underline">Log in</a
                >
            </p>
        </form>
    </div>
